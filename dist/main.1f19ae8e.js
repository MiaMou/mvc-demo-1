...
$number.text(n || 100); // 然后绑定事件

$button1.on('click', function () {
  var n = parseInt($number.text());
  n += 1; // 有数据存储功能的n (当刷新页面的时候保留最新的n值)
...
var $tabContent = (0, _jquery.default)('#app2 .tab-content'); // jquery可以通过监听父元素来监听子元素的事件（事件委托）

$tabBar.on('click', 'li', function (e) {
  // 通过jquery来操作元素
  var $li = (0, _jquery.default)(e.currentTarget); // 添加选择器实现背景色转换

  $li.addClass('selected').siblings().removeClass('selected');
  var index = $li.index(); // 添加选择器实现切换内容
